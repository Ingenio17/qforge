...
def main():
    print("============================================================")
    print(" Example 11: Deutsch Algorithm (Balanced Oracle)")
    print("============================================================")
    ...
    best_cnot_dur, max_p11 = g_eng.calibrate_gate(
        ...
        parameter="duration", 
        range_vals=np.linspace(20, 300, 20)
    )
    ...
    print(f"\n[EVOLUTION]: Simulating Deutsch Algorithm Sequence")
    ...
    TerminalPlotter.plot_time_evolution(
        ...
        labels=['P(|01>) (Init)', 'P(|11>) (Final)', 'Leakage'],
        title="Deutsch Algorithm Evolution (Balanced Oracle)",
        ylim=(0, 1.05)
    )
    
    # Save high-res plot to outputs/plots
    os.makedirs(os.path.join("qforge", "outputs", "plots"), exist_ok=True)
    save_path = os.path.join("qforge", "outputs", "plots", "11_deutsch_algorithm.png")
    ...
    # Add phase markers
    plt.axvline(x=t_offset_1, color='gray', linestyle=':', label='H -> CNOT')
    plt.axvline(x=t_offset_2, color='gray', linestyle='-.', label='CNOT -> H')
    
    plt.xlabel('Time (ns)')
    plt.ylabel('Population')
    plt.title('Deutsch Algorithm (Balanced Oracle) Evolution')
    plt.legend()
